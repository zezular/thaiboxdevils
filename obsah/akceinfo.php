...
<body>
    ...
    <section id="testimonial2">
        <div class="container">
				...
				$idakce=$_GET['akce'];
				$prikaz="SELECT * FROM akce WHERE id='$idakce'";
			...
			while($pole = mysqli_fetch_assoc($vysledek)){
    			...
    			<?php 
     			...
     			$idakce=$_GET['akce']; 
    			$query = $db->query("SELECT * FROM akce WHERE textobrazku='$idakce' "); 
     			if($query->num_rows > 0){ 
      			while($row = $query->fetch_assoc()){ 
           			 $imageThumbURL = '../admin/web/akce_thumb/'.$row["file_name"]; 
           			 $imageURL = '../admin/web/akce_fotky/'.$row["file_name"]; 
   				 ?>
...
<?php } }?> 
	<br><br><a href='akce.php'><strong><p>Zpět</p></strong></a>						 
</div>
